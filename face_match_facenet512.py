...
from config import (FACE_IMAGE_DIRECTORY, FINAL_FACES_DIRECTORY,
                    FACENET512_MATCH_RESULT_CSV_PATH,
                    FACENET512_MATCH_SAVE_IMAGES_DIRECTORY)
...
class MATCHFACE():
    def __init__(self):
        ...
        if os.path.isfile(FACENET512_MATCH_RESULT_CSV_PATH):
            try:
                self.face_match_dataframe = pd.read_csv(FACENET512_MATCH_RESULT_CSV_PATH)
            except Exception as e:
                print(f"{FACENET512_MATCH_RESULT_CSV_PATH} already present.")
        ...
        for face_image_path in face_images_list:
            ...
            if face_image_path[-4:] == ".png":
                ...
                if not is_value_present_flag:
                    print("--" * 200)
                    dfs = DeepFace.find(img_path = face_image_path, db_path = FINAL_FACES_DIRECTORY, distance_metric="cosine", model_name="Facenet512", enforce_detection=False)
                    ...
                    self.face_match_dataframe.to_csv(FACENET512_MATCH_RESULT_CSV_PATH, index=False)

                    ic(video_id)
                    for i in range (0, len(match_face_paths)):
                        individual_face_match_directory = f"{FACENET512_MATCH_SAVE_IMAGES_DIRECTORY}/{video_id}"
                        if not os.path.isdir(individual_face_match_directory):
                            os.mkdir(individual_face_match_directory)

                        match_file_bid, match_video_id, match_file_name_suffix = get_metadata_from_file_name(match_face_paths[i].split("/")[-1])
