...
class MATCHFACE():
    def __init__(self):
        ...
        if os.path.isfile(VGG_FACE_MATCH_RESULT_CSV_PATH):
            try:
                self.face_match_dataframe = pd.read_csv(VGG_FACE_MATCH_RESULT_CSV_PATH)
            except Exception as e:
                print(f"{VGG_FACE_MATCH_RESULT_CSV_PATH} already present.")
        ...
        for face_image_path in face_images_list:
            ...
            if face_image_path[-4:] == ".png":
                ...
                if not is_value_present_flag:
                    ...
                    self.face_match_dataframe.to_csv(VGG_FACE_MATCH_RESULT_CSV_PATH, index=False)

                    ic(video_id)
                    for i in range (0, len(match_face_paths)):
                        individual_face_match_directory = f"{VGG_FACE_MATCH_SAVE_IMAGES_DIRECTORY}/{video_id}"
